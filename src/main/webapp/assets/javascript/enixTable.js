$.fn.enixTable = function(customOptions) {
    ...
    let enixTableFunc = {
        initEnixTable : function () {
			...
			//상신함에서 결재회수 버튼 노출
            if(options.isWithdraw){
				$(theadRow).append('<th class="aCenter">결재회수</th>');
			}
            ...
            $.each(options.data, function (index, rowData) {
                ...
                $.each(options.displayMember, function (index2, dpMember) {
					...
					if (options.switchMember.includes(dpMember)) {
                        ...
                        // 날짜인 경우 문자열 변환
                        if(dpMember.includes('createDate') || dpMember.includes('updateDate') || dpMember.includes('creationDate')){
						...
						// 제목인 경우 팝업
                        if(dpMember == 'docName'){
							_colData = '<td class="{0}"><a href="#" onclick="showDetail(\'{1}\');" class="titleCollapse">{2}</a></td>'.format(options.colAlign[index2], rowData["docId"], _colData);
				...
				//상신함에서 결재회수 버튼 노출
	            if(options.isWithdraw){
					$(tbodyRow).append('<td class="aCenter"><a onclick="evtWorkflowStep(\'{0}\',\'{1}\',\'WITHDRAW\');" class="txt-red">결재회수</a></td>'.format(rowData["docId"],rowData["workflowId"]));
				}
    ...
    return {
        ...
        setPermissionData : function(dataList) {
			...
			$.each(dataList, function (index, item) {
                ...
                _colData =(
                            ...
                            '	<span class="btn_access_drm_{8}">인쇄</span>'+
                            '</div>'+
                            '</td></tr>'
                        ).format(item.targetType, item.targetName, item.listView,item.read,item.update,item.delete,item.download,item.edit,item.print);
        ...
        // 결재대상 목록을 출력한다. @문서상제정보  
        setWorkflowItems : function(dataList) {
                ...
                _colData =(
                            '<tr style="background-color: #e0f3ff">'+
	                            ...
	                            '<td class="text-center"><div>'+
		                            '<button class="btn btn-xsm btn-primary" href="javascript:void();" onclick="_docDetail.evtWorkflowStep(\'{0}\',\'APPROVAL\');">승인</button>&nbsp;'+
		                            '<button class="btn btn-xsm btn-red" onclick="_docDetail.evtWorkflowStep(\'{0}\',\'REJECTION\');">반려</button>&nbsp;'+
	                            '</div></td>' +
                            '</tr>'+
                            '<tr>'+
	                            '<td colspan="4" class="text-left"><textarea class="workflowDesc" disabled>{4}</textarea></td>' +
                            '</tr>'
                        ).format(item.workflowId, item.workflowTypeName, item.creationDate, item.creatorInfo,item.creatorMessage);
                $(tbody).append(_colData);
            });
        },
